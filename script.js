function mostrarModulos() {
  ...
  openTab(
    { currentTarget: document.querySelector(".tab-button:nth-child(1)") },
    "atenuacion"
  ); // Open Atenuación y Ganancia
}
...
function dragStart(event) {
  event.dataTransfer.setData(
    "tipo",
    event.target.className.includes("atenuacion") ? "atenuacion" : "ganancia"
  );
}
...
function calcularSumaLogaritmica() {
  ...
  const sumaPotenciasLineales = potenciasLineales.reduce(
    (total, potencia) => total + potencia,
    0
  );
  ...
  mostrarPasoAPasoLogaritmico(
    potenciasLineales,
    sumaPotenciasLineales,
    resultadoEnDb
  );
}

function mostrarPasoAPasoLogaritmico(
  potenciasLineales,
  sumaPotenciasLineales,
  resultadoEnDb
) {
  ...
  potenciasLineales.forEach((potencia, index) => {
    ...
    detallePaso1.textContent = `   - Para el valor ${
      ajustes[index]
    } dB: Potencia Lineal = ${potencia.toFixed(4)}`;
  ...
  detallePaso2.textContent = `   - Suma de Potencias Lineales = ${sumaPotenciasLineales.toFixed(
    4
  )}`;
  ...
  detallePaso3.textContent = `   - Resultado en dB = ${resultadoEnDb.toFixed(
    2
  )} dB`;
...
function calcularTiempoTransmision() {
  const tamanoMensaje = parseFloat(
    document.getElementById("tamanoMensaje").value
  );
  const velocidadTransmision = parseFloat(
    document.getElementById("velocidadTransmision").value
  );
  ...
  if (
    isNaN(tamanoMensaje) ||
    isNaN(velocidadTransmision) ||
    tamanoMensaje <= 0 ||
    velocidadTransmision <= 0
  ) {
    alert(
      "Por favor, ingrese valores válidos para el tamaño del mensaje y la velocidad de transmisión."
    );
    return;
  }
  ...
  if (!unidadTamano || !unidadVelocidad) {
    alert(
      "Por favor, seleccione las unidades para el tamaño del mensaje y la velocidad de transmisión."
    );
    return;
  }
  ...
  document.getElementById("tiempoTransmision").textContent = `${tiempo.toFixed(
    4
  )} segundos`; // Update styled div

  // Mostrar el paso a paso
  mostrarPasoAPasoTransmision(
    tamanoMensaje,
    unidadTamano,
    bitsMensaje,
  ...
  document
    .getElementById("pasoAPasoTransmision")
...
function mostrarPasoAPasoTransmision(
  tamanoMensaje,
  unidadTamano,
  bitsMensaje,
  velocidadTransmision,
  ...
  resultadoPaso3.textContent = `   - Tiempo de transmisión = ${bitsMensaje} / ${bitsPorSegundo} = ${tiempo.toFixed(
    4
  )} segundos.`;
...
function calcularResultado() {
  try {
    ...
    if (
      ...
      pasos.push(
        `Operación directa: ${valor1} dBm ${operador} ${valor2} dB = ${resultado.toFixed(
          2
        )} dBm`
      );
    } else if (
      ...
      resultado = operador === "+" ? valor1 + valor2 : valor1 - valor2;
      pasos.push(
        `Operación directa: ${valor1} dBW ${operador} ${valor2} dB = ${resultado.toFixed(
          2
        )} dBW`
      );
    } else if (
      (unidad1 === "dBm" || unidad1 === "dBW") &&
      (unidad2 === "dBm" || unidad2 === "dBW")
    ) {
      ...
      pasos.push(
        `Convertimos a mW: ${valor1} ${unidad1} → ${mW1.toFixed(4)} mW`
      );
      pasos.push(
        `Convertimos a mW: ${valor2} ${unidad2} → ${mW2.toFixed(4)} mW`
      );
      pasos.push(
        `Realizamos la operación en mW: ${mW1.toFixed(
          4
        )} ${operador} ${mW2.toFixed(4)} = ${operacionLineal.toFixed(4)} mW`
      );
      pasos.push(
        `Convertimos el resultado de mW a ${unidad1}: ${operacionLineal.toFixed(
          4
        )} mW → ${resultado.toFixed(2)} ${unidad1}`
      );
    } else if (unidad1 === "dB" || unidad2 === "dB") {
      display.value = "Operación inválida";
      pasos.push(
        "No se puede operar dos valores relativos (dB) ni hacer operaciones absolutas con dB sin base."
      );
    ...
    historial.innerHTML += `<div>${expresion} = ${resultado.toFixed(
      2
    )} ${baseUnit}</div>`;
    expresion = "";
...
function obtenerColorPorTipo(tipo) {
  switch (tipo) {
    case "potenciaTransmitida":
      return "red";
    case "gananciaTransmisora":
      return "green";
    case "gananciaReceptora":
      return "blue";
    case "distancia":
      return "orange";
    case "frecuencia":
...
function calcularFSPL() {
  ...
  const fspl =
    ...
    20 * Math.log10(frecuencia) -
    20 * Math.log10(c);

  document.getElementById("resultadoEnlaceRadio").innerHTML = `
        <p><strong>Pérdida en el Espacio Libre (FSPL):</strong> ${fspl.toFixed(
          2
        )} dB</p>
    `;
...
function calcularPotenciaRecibida() {
  ...
  if (pt === null || gt === null || gr === null || d === null || f === null) {
    alert(
      "Faltan datos: asegúrate de ingresar todos los parámetros necesarios."
    );
    return;
  }
...
function generarGraficoAM() {
  ...
  if (
    ...
    alert(
      "Por favor, ingrese valores válidos para las frecuencias y el índice de modulación."
    );
    return;
  }
...
let chartPortadora, chartModuladora, chartModulada, chartEnvolvente; // Store chart instances
...
function calcularFrecuenciaModulada() {
  ...
  const portadora = t.map(
    (tiempo) =>
      amplitudPortadora * Math.sin(2 * Math.PI * frecuenciaPortadora * tiempo)
  );
  const moduladora = t.map(
    (tiempo) =>
      amplitudModuladora * Math.sin(2 * Math.PI * frecuenciaModuladora * tiempo)
  );
...
function graficar(canvasId, t, data, label) {
...
function graficarModuladoraYModulada(canvasId, t, moduladora, modulada, label) {
  const canvas = document.getElementById(canvasId);
  const context = canvas.getContext("2d");
...
function generarGraficoASKFSK() {
  const datosBinarios = document.getElementById("datosBinarios").value.trim();
  const frecuenciaPortadoraASK = parseFloat(
    document.getElementById("frecuenciaPortadoraASK").value
  );
  const frecuenciaFSK1 = parseFloat(
    document.getElementById("frecuenciaPortadoraFSK1").value
  );
  const frecuenciaFSK0 = parseFloat(
    document.getElementById("frecuenciaPortadoraFSK0").value
  );

  if (
    !/^[01]+$/.test(datosBinarios) ||
    isNaN(frecuenciaPortadoraASK) ||
    isNaN(frecuenciaFSK1) ||
    isNaN(frecuenciaFSK0) ||
    frecuenciaPortadoraASK <= 0 ||
    frecuenciaFSK1 <= 0 ||
    frecuenciaFSK0 <= 0
  ) {
    alert("Por favor, ingrese datos válidos para las entradas.");
    return;
  }

  // Crear un vector de tiempo
  const bits = datosBinarios.split("").map(Number);
  const samplesPerBit = 100; // Número de muestras por bit
  const t = Array.from(
    { length: bits.length * samplesPerBit },
    (_, i) => i / samplesPerBit
  );

  // Generar señal ASK
  const ask = t.map((tiempo, i) => {
    const bitIndex = Math.floor(i / samplesPerBit);
    return bits[bitIndex] === 1
      ? Math.sin(2 * Math.PI * frecuenciaPortadoraASK * tiempo)
      : 0;
  });

  // Generar señal FSK
  const fsk = t.map((tiempo, i) => {
    const bitIndex = Math.floor(i / samplesPerBit);
    const frecuencia =
      bits[bitIndex] === 1 ? frecuenciaFSK1 : frecuenciaFSK0;
    return Math.sin(2 * Math.PI * frecuencia * tiempo);
  });

  // Limpiar gráficos previos si existen
  if (window.chartASK) window.chartASK.destroy();
  if (window.chartFSK) window.chartFSK.destroy();
